...
class Test extends Thread
{
    ...
    /**
     * Change the UID to run the process with.
     * 
     * @var string
     */
    protected $uid;
     ...
     * @param string   $uid    The UID to run the process under
     * @return void
     */
    ...
    public function run()
    {
        ...
        do {
            ...
            if ($buffer === '') {
                ...
                // prepare HTML body
                $body = '<html><head><title>A Title</title></head><body><p>Generated by thread: ' . $threadId . '</p><p>' . var_export($_SESSION, true) . '</p>' . posix_getuid() . '</body></html>';
...
if ($socket) {
    ...
    // initialize the UID, default is 'nobody'
    $uid = 'nobody';
    if (isset($argv[2])) {
        $uid = $arv[2];
    }
    ...
    while (++ $worker < 5) {
        $workers[$worker] = new Test($socket, $uid);
